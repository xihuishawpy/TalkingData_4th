...
def read_csv(csv_file,df_len=None,nrows=None,usecols=None,dtype=None):
    ...
    if os.path.isfile(pkl_file) and nrows is None:
        ...
        if nrows is None and (df_len is None or len(df) == df_len):
        ...
        if nrows is None and df_len and len(df) != df_len:
...
def read_data_ph1():
    ...
    tgt += '_ver=3'
    tr_pkl_file = f'../work/train_{tgt}.pkl'
    te_pkl_file = f'../work/test_supplement_{tgt}.pkl'
    ...
    for n, ptn in enumerate(cat_patterns, start=1):
        ...
        max_val = _df[ptn].max()
        if ptn == 'cat_device' and get_opt('noLogDev', '-') == 'on':
            _df[ptn] = LabelEncoder().fit_transform(_df[ptn])
        elif thre_val > 0 and max_val > thre_val:
            fixed_vals = (
                (10000 * df[ptn]).astype('uint16')
                if 'cumratio' in ptn
                else (np.log2(_df[ptn] + 1) * thre_val / 100).astype('uint16')
            )
    ...
    for ptn in numerical_patterns:
        if get_opt('model','-') == 'keras':
            ...
            if (
                'cumratio' not in ptn
                and 'CVRTgt' not in ptn
                and 'WOETgt' not in ptn
            ):
